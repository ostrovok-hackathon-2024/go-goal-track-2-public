...
CATBOOST_API void AddFloatFeatures(DataWrapperHandle *dataWrapperHandle,
                                   const float **floatFeatures,
                                   size_t floatFeaturesSize);

CATBOOST_API void AddCatFeatures(DataWrapperHandle *dataWrapperHandle,
                                 const char ***catFeatures,
                                 size_t catFeaturesSize);

CATBOOST_API void AddTextFeatures(DataWrapperHandle *dataWrapperHandle,
                                  const char ***textFeatures,
                                  size_t textFeaturesSize);

CATBOOST_API void AddEmbeddingFeatures(DataWrapperHandle *dataWrapperHandle,
                                       const float ***embeddingFeatures,
                                       size_t *embeddingDimensions,
                                       size_t embeddingFeaturesSize);

CATBOOST_API DataProviderHandle *
BuildDataProvider(DataWrapperHandle *dataWrapperHandle);
 ...
 */
CATBOOST_API bool LoadFullModelFromFile(ModelCalcerHandle *modelHandle,
                                        const char *filename);
 ...
 */
CATBOOST_API bool EnableGPUEvaluation(ModelCalcerHandle *modelHandle,
                                      int deviceId);
 ...
 * @param formulaEvaluatorTypes address of the pointer to an array that will be
 * initialized with formula evaluator types
 * @param formulaEvaluatorTypesCount address of the variable where the size of
 * formulaEvaluatorTypes array will be stored
 ...
 */
CATBOOST_API bool SetPredictionType(ModelCalcerHandle *modelHandle,
                                    enum EApiPredictionType predictionType);

/**
 * Set prediction type for model evaluation with string constant
 */
CATBOOST_API bool SetPredictionTypeString(ModelCalcerHandle *modelHandle,
 ...
 * Flat here means that float features and categorical feature are in the same
 * float array.
 * @param calcer model handle
 * @param docCount number of objects
 * @param floatFeatures array of array of float (first dimension is object
 * index, second is feature index)
 ...
 * docCount (e.g. for non multiclass models should be equal to docCount)
 * @return false if error occured
 */
CATBOOST_API bool CalcModelPredictionFlat(ModelCalcerHandle *modelHandle,
                                          size_t docCount,
                                          const float **floatFeatures,
                                          size_t floatFeaturesSize,
 ...
 * @param calcer model handle
 * @param docCount number of objects
 * @param treeStart the index of the first tree to be used when applying the
 * model (zero-based)
 * @param treeEnd the index of the last tree to be used when applying the model
 * (non-inclusive, zero-based)
 * @param floatFeatures array of array of float (first dimension is object
 * index, second is feature index)
 * @param floatFeaturesSize float values array size
 * @param result pointer to user allocated results vector
...
CATBOOST_API bool CalcModelPredictionFlatStaged(
    ...
    size_t treeEnd, const float **floatFeatures, size_t floatFeaturesSize,
    double *result, size_t resultSize);
 ...
 * @param calcer model handle
 * @param docCount number of objects
 * @param floatFeatures array of array of float (first dimension is feature
 * index, second is object index)
 * @param floatFeaturesSize float values array size
 * @param result pointer to user allocated results vector
 * @param resultSize Result size should be equal to modelApproxDimension *
 * docCount (e.g. for non multiclass models should be equal to docCount)
 ...
 */
CATBOOST_API bool
CalcModelPredictionFlatTransposed(ModelCalcerHandle *modelHandle,
                                  size_t docCount, const float **floatFeatures,
                                  size_t floatFeaturesSize, double *result,
                                  size_t resultSize);
...
/**
 * Calculate raw model predictions on float features and string categorical
 * feature values
 * @param calcer model handle
 ...
 */
CATBOOST_API bool
CalcModelPrediction(ModelCalcerHandle *modelHandle, size_t docCount,
...
/**
 * Calculate raw model predictions on float features and string categorical
 * feature values taking into consideration only the trees in the range
 * [treeStart; treeEnd)
 * @param calcer model handle
 * @param docCount object count
 * @param treeStart the index of the first tree to be used when applying the
 ...
 */
CATBOOST_API bool
CalcModelPredictionStaged(ModelCalcerHandle *modelHandle, size_t docCount,
 ...
 */
CATBOOST_API bool
CalcModelPredictionText(ModelCalcerHandle *modelHandle, size_t docCount,
                        const float **floatFeatures, size_t floatFeaturesSize,
                        const char ***catFeatures, size_t catFeaturesSize,
...
CATBOOST_API bool CalcModelPredictionTextStaged(
    ...
    size_t treeEnd, const float **floatFeatures, size_t floatFeaturesSize,
    const char ***catFeatures, size_t catFeaturesSize,
    const char ***textFeatures, size_t textFeaturesSize, double *result,
    size_t resultSize);
 ...
 * String pointer should point to zero terminated string.
 * @param textFeaturesSize text feature count
 * @param embeddingFeatures array of array of array of float (first dimension is
 * object index, second is feature index, third is index in embedding array).
 * String pointer should point to zero terminated string.
...
CATBOOST_API bool CalcModelPredictionTextAndEmbeddings(
    ...
    const char ***catFeatures, size_t catFeaturesSize,
    const char ***textFeatures, size_t textFeaturesSize,
    const float ***embeddingFeatures, size_t *embeddingDimensions,
    size_t embeddingFeaturesSize, double *result, size_t resultSize);
...
/**
 * Calculate raw model prediction on float features and string categorical
 * feature values for single object
 * @param calcer model handle
 ...
 * @return false if error occured
 */
CATBOOST_API bool CalcModelPredictionSingle(ModelCalcerHandle *modelHandle,
                                            const float *floatFeatures,
                                            size_t floatFeaturesSize,
                                            const char **catFeatures,
                                            size_t catFeaturesSize,
...
/**
 * Calculate raw model prediction on float features and string categorical
 * feature values for single object taking into consideration only the trees in
 * the range [treeStart; treeEnd)
 * @param calcer model handle
 * @param treeStart the index of the first tree to be used when applying the
 * model (zero-based)
 * @param treeEnd the index of the last tree to be used when applying the model
...
CATBOOST_API bool CalcModelPredictionSingleStaged(
    ModelCalcerHandle *modelHandle, size_t treeStart, size_t treeEnd,
    const float *floatFeatures, size_t floatFeaturesSize,
    const char **catFeatures, size_t catFeaturesSize, double *result,
    size_t resultSize);

/**
 * Calculate raw model predictions on float features and hashed categorical
 * feature values
 * @param calcer model handle
 * @param docCount object count
 * @param floatFeatures array of array of float (first dimension is object
 * index, second if feature index)
 * @param floatFeaturesSize float feature count
 * @param catFeatures array of array of integers - hashed categorical feature
 * values.
 * @param catFeaturesSize categorical feature count
 * @param result pointer to user allocated results vector
 * @param resultSize result size should be equal to modelApproxDimension *
...
CATBOOST_API bool CalcModelPredictionWithHashedCatFeatures(
    ModelCalcerHandle *modelHandle, size_t docCount,
    const float **floatFeatures, size_t floatFeaturesSize,
    const int **catFeatures, size_t catFeaturesSize, double *result,
    size_t resultSize);

CATBOOST_API bool CalcModelPredictionWithHashedCatFeaturesAndTextFeatures(
    ModelCalcerHandle *modelHandle, size_t docCount,
    const float **floatFeatures, size_t floatFeaturesSize,
    const int **catFeatures, size_t catFeaturesSize, const char ***textFeatures,
    size_t textFeaturesSize, double *result, size_t resultSize);
...
CalcModelPredictionWithHashedCatFeaturesAndTextAndEmbeddingFeatures(
    ...
    const float **floatFeatures, size_t floatFeaturesSize,
    const int **catFeatures, size_t catFeaturesSize, const char ***textFeatures,
    size_t textFeaturesSize, const float ***embeddingFeatures,
    size_t *embeddingDimensions, size_t embeddingFeaturesSize, double *result,
    size_t resultSize);
 ...
 */
CATBOOST_API bool
PredictSpecificClassFlat(ModelCalcerHandle *modelHandle, size_t docCount,
                         const float **floatFeatures, size_t floatFeaturesSize,
                         int classId, double *result, size_t resultSize);

CATBOOST_API bool
PredictSpecificClass(ModelCalcerHandle *modelHandle, size_t docCount,
                     const float **floatFeatures, size_t floatFeaturesSize,
                     const char ***catFeatures, size_t catFeaturesSize,
                     int classId, double *result, size_t resultSize);

CATBOOST_API bool
PredictSpecificClassText(ModelCalcerHandle *modelHandle, size_t docCount,
                         const float **floatFeatures, size_t floatFeaturesSize,
                         const char ***catFeatures, size_t catFeaturesSize,
                         const char ***textFeatures, size_t textFeaturesSize,
                         int classId, double *result, size_t resultSize);
...
CATBOOST_API bool PredictSpecificClassTextAndEmbeddings(
    ...
    const char ***textFeatures, size_t textFeaturesSize,
    const float ***embeddingFeatures, size_t *embeddingDimensions,
    size_t embeddingFeaturesSize, int classId, double *result,
    size_t resultSize);

CATBOOST_API bool
PredictSpecificClassSingle(ModelCalcerHandle *modelHandle,
                           const float *floatFeatures, size_t floatFeaturesSize,
                           const char **catFeatures, size_t catFeaturesSize,
                           int classId, double *result, size_t resultSize);
...
CATBOOST_API bool PredictSpecificClassWithHashedCatFeatures(
    ModelCalcerHandle *modelHandle, size_t docCount,
    const float **floatFeatures, size_t floatFeaturesSize,
    const int **catFeatures, size_t catFeaturesSize, int classId,
    double *result, size_t resultSize);
...
CATBOOST_API bool PredictSpecificClassWithHashedCatFeaturesAndTextFeatures(
    ...
    const float **floatFeatures, size_t floatFeaturesSize,
    const int **catFeatures, size_t catFeaturesSize, const char ***textFeatures,
    size_t textFeaturesSize, int classId, double *result, size_t resultSize);

CATBOOST_API bool
PredictSpecificClassWithHashedCatFeaturesAndTextAndEmbeddingFeatures(
    ModelCalcerHandle *modelHandle, size_t docCount,
    const float **floatFeatures, size_t floatFeaturesSize,
    const int **catFeatures, size_t catFeaturesSize, const char ***textFeatures,
    size_t textFeaturesSize, const float ***embeddingFeatures,
    size_t *embeddingDimensions, size_t embeddingFeaturesSize, int classId,
    double *result, size_t resultSize);
 ...
 * @return true on success, false on error
 */
CATBOOST_API bool GetFloatFeatureIndices(ModelCalcerHandle *modelHandle,
 ...
 * @return true on success, false on error
 */
CATBOOST_API bool GetCatFeatureIndices(ModelCalcerHandle *modelHandle,
                                       size_t **indices, size_t *count);
 ...
 * @return true on success, false on error
 */
CATBOOST_API bool GetTextFeatureIndices(ModelCalcerHandle *modelHandle,
                                        size_t **indices, size_t *count);
 ...
 * @return true on success, false on error
 */
CATBOOST_API bool GetEmbeddingFeatureIndices(ModelCalcerHandle *modelHandle,
                                             size_t **indices, size_t *count);
 ...
 * For default `APT_RAW_FORMULA_VAL`, `APT_EXPONENT`, `APT_PROBABILITY`,
 * `APT_CLASS` prediction type GetPredictionDimensionsCount ==
 * GetDimensionsCount For `APT_RMSE_WITH_UNCERTAINTY` - returns 2 (value
 * prediction and predicted uncertainty)
 * @param calcer model handle
 */
CATBOOST_API size_t
GetPredictionDimensionsCount(ModelCalcerHandle *modelHandle);

/**
 * Check if model metadata holds some value for provided key
 * @param calcer model handle
 */
CATBOOST_API bool CheckModelMetadataHasKey(ModelCalcerHandle *modelHandle,
                                           const char *keyPtr, size_t keySize);

/**
 * Get model metainfo value size for some key. Returns 0 both if key is missing
 * in model metadata and if it is really missing
 * @param calcer model handle
 */
CATBOOST_API size_t GetModelInfoValueSize(ModelCalcerHandle *modelHandle,
                                          const char *keyPtr, size_t keySize);

/**
 * Get model metainfo for some key. Returns const char* pointer to inner string.
 * If key is missing in model metainfo storage this method will return nullptr
 * @param calcer model handle
 */
CATBOOST_API const char *GetModelInfoValue(ModelCalcerHandle *modelHandle,
                                           const char *keyPtr, size_t keySize);

/**
 * Get names of features used in the model.
 * individual strings in featureNames array and featureNames array itself must
 * be deallocated using free() after use.
 *
 * @return true on success, false on error
 */
CATBOOST_API bool GetModelUsedFeaturesNames(ModelCalcerHandle *modelHandle,
                                            char ***featureNames,
                                            size_t *featureCount);
